...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("interact") and is_near_npc:
		if Dialogic.current_timeline == null:
			...
			Dialogic.start("2justianorizal1")
...
func _on_dialogic_ended() -> void:
	...
	if Dialogic.VAR.justianorizalmultiplechoice.justianomultiplechoicefinished == true:
...
func _on_chatdetectionjustiano_body_exited(body: Node2D) -> void:
	if body is Player:
		is_near_npc = false
